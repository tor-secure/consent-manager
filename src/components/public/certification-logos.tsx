  ...
  return (
    <div className={compact ? "mt-0" : "mt-8"}>
      <p
        className={
          compact
            ? "text-left text-[11px] font-semibold leading-snug text-[#0B2C4A] sm:text-xs"
      ...
      <ul
        className={
          compact
            ? "mt-2 grid w-full grid-cols-6 items-center justify-items-center gap-x-2 gap-y-3 lg:flex lg:flex-nowrap lg:justify-between lg:gap-x-3"
        ...
        {CERTIFICATIONS.map((item, index) => {
          const image = (
            <Image
              ...
              className={
                item.kind === "badge"
                  ? compact
                    ? "h-9 w-9 object-contain lg:h-11 lg:w-11"
                    : "h-12 w-12 object-contain sm:h-14 sm:w-14"
                  : compact
                    ? "h-7 w-auto max-w-[92px] object-contain lg:h-8 lg:max-w-[120px]"
          ...
          return (
            <li
              key={item.src}
              className={[
                "flex items-center justify-center lg:shrink-0 lg:justify-start",
