    ...
    <main className="mx-auto max-w-lg px-4 py-16">
      ...
      <form onSubmit={submit} className="mt-6 space-y-4">
        <div className="flex gap-2">
          <button type="button" onClick={() => setMode("status")} className={`rounded-2xl px-3 py-1.5 text-xs ${mode === "status" ? "bg-slate-900 text-white" : "border border-slate-200"}`}>
            Check status
          </button>
          <button type="button" onClick={() => setMode("verify")} className={`rounded-2xl px-3 py-1.5 text-xs ${mode === "verify" ? "bg-slate-900 text-white" : "border border-slate-200"}`}>
            Verify identity
          </button>
        </div>
        <input
