...
export async function POST(
  ...
  try {
    ...
    if (existing.status === "resolved") {
      return NextResponse.json(
        { success: false, message: "Resolved findings cannot be marked reviewed." },
        { status: 400 },
      );
    }
    ...
    await db.insert(auditLogs).values({
      ...
      userId: localUser.id,
      action: "privacy_finding.reviewed",
      resourceType: "privacy_finding",
      resourceId: existing.id,
      description: "Privacy finding marked reviewed",
      metadata: { websiteId: existing.websiteId, previousStatus: existing.status },
    });

    return NextResponse.json({ success: true });
  } catch {
    return NextResponse.json({ success: false, message: "Unable to review finding." }, { status: 500 });
  }
}
